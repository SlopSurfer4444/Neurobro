...
function message(value: unknown, binding: PilotBinding): StandingContextMessage {
  const v = record(value, ["chatId", "messageId", "authorId", "author", "displayName", "date", "replyToMessageId", "text"]);
  ...
  return { chatId: binding.peerId, messageId: id, authorId, author: v.author as "user" | "self", displayName: text(v.displayName, 512),
    date: date(v.date), replyToMessageId: reply, text: text(v.text, 4096) };
}
...
export function readStandingSharedContext(input: StandingSharedContextReaderInput): BoundStandingSharedContext {
  ...
  const primary: PilotPrimary = { chatId: binding.peerId, ownerId: userId(p.ownerId), messageId: messageId(p.messageId), text: text(p.text, 4096) };
  ...
  if (Object.hasOwn(v, "context")) {
    ...
    if (current.messageId !== primary.messageId || current.authorId !== primary.ownerId || current.author !== "user" || current.text !== primary.text) return fail();
  ...
  if (Object.hasOwn(v, "restoration")) {
    ...
    if (r.status === "unavailable") dialogueCoverage = unavailable("unavailable");
      ...
      for (const pair of [...r.dialogues].reverse()) {
        ...
        if (current && (current.text !== pair.question.text || references.speaker(current.authorId) !== pair.question.speaker ||
