...
export type StandingHistoryAnalysisAttemptPlan = PlanBase & (
  ...
  Readonly<{ kind: "merge"; children: readonly string[] }>);
...
const DOMAIN = "DecadansNeurobro/standing-history-analysis-attempt/v1", MAX_ATTEMPTS = 1024, MAX_PLAIN = 131072, MAX_CIPHER = 196608;
...
function material(v: unknown): StandingHistoryAnalysisMaterialRequest {
  ...
  if (!Number.isInteger(m.pageIndex) || Number(m.pageIndex) < 1 || Number(m.pageIndex) > 1024 || !Number.isInteger(m.maxBytes) || Number(m.maxBytes) < 1024 || Number(m.maxBytes) > 49152 || !ref(m.materialRef, "hmat") ||
...
function planCopy(v: unknown): StandingHistoryAnalysisAttemptPlan {
  const p = data(v, ["kind", "sourceHead", "expectedHead", "nodeIndex", "modelInputHash"], ["inputs", "children"]);
  if (!digest(p.sourceHead) || !digest(p.expectedHead) || !digest(p.modelInputHash) || !Number.isInteger(p.nodeIndex) || Number(p.nodeIndex) < 1 || Number(p.nodeIndex) > 1024) return fail("input");
  ...
  if (p.kind === "leaf" && !Object.hasOwn(p, "children")) {
    const inputs = array(p.inputs, 8, 1).map(material); if (new Set(inputs.map(m => m.materialRef)).size !== inputs.length) return fail("input");
    return Object.freeze({ kind: "leaf", ...base, inputs: Object.freeze(inputs) });
  }
  if (p.kind === "merge" && !Object.hasOwn(p, "inputs")) {
    const children = array(p.children, 8, 1); if (children.some(v => !ref(v, "hnode")) || new Set(children).size !== children.length) return fail("input");
    return Object.freeze({ kind: "merge", ...base, children: Object.freeze(children as string[]) });
  }
  return fail("input");
}
...
type Attempt = { attemptRef: string; index: number; plan: StandingHistoryAnalysisAttemptPlan; planHash: string; reservationHash: string; nativeBinding?: StandingHistoryAnalysisNativeBinding;
...
export async function openStandingHistoryAnalysisAttemptStore(input: Readonly<{
  ...
  const previousHash = (a: Attempt | undefined) => {
    if (!a) return intentHash; if (!a.prepared || !a.node || !a.native) return fail("consumed");
  ...
  const reservationRecord = (a: Attempt, previous: string) => ({ domain: DOMAIN, taskId: intent.taskId, kind: "reservation", intentHash,
    ...
    ...(a.nativeBinding ? { nativeBinding: a.nativeBinding } : {}) });
  ...
  const evidenceCopy = (value: unknown): StandingHistoryAnalysisNodeEvidence => {
    const e = data(value, ["nodeRef", "index", "hash"]); if (!ref(e.nodeRef, "hnode") || !digest(e.hash) || !Number.isInteger(e.index) || Number(e.index) < 1 || Number(e.index) > 1024) return fail("binding");
  ...
  const analysisSnapshot = async () => {
    ...
    if (s.storage !== "ready" || !digest(s.headHash) || !Number.isInteger(s.analysisNodes) || Number(s.analysisNodes) < 0 || Number(s.analysisNodes) > 1024) return fail("storage");
  ...
  const verifyNode = (value: StandingHistoryAnalysisNode, a: Attempt): StandingHistoryAnalysisNodeEvidence => {
    ...
    if (a.plan.kind === "leaf") {
      ...
      const inputs = array(i.materials, 8, 1).map(value => {
      ...
      const i = data(n.inputs, ["children"]), children = array(i.children, 8, 1); if (!equal(children, a.plan.children)) return fail("binding");
    }
    return evidence;
  };
  try {
    ...
    if (args.mode === "create") await write("intent.enc", header);
      ...
      for (let index = 1; index <= MAX_ATTEMPTS && listing.names.has(filename(index, "reservation")); index++) {
        try {
          const previous = attempts.at(-1), priorHash = previousHash(previous), savedReservation = await read(filename(index, "reservation"));
          const r = data(savedReservation.value, ["domain", "taskId", "kind", "intentHash", "attemptIndex", "attemptRef", "previousHash", "plan", "planHash"], ["nativeBinding"]);
          ...
          const plan = planCopy(r.plan), a: Attempt = { attemptRef: r.attemptRef, index, plan, planHash: hash(plan), reservationHash: hash(savedReservation.value),
            ...(Object.hasOwn(r, "nativeBinding") ? { nativeBinding: nativeBindingCopy(r.nativeBinding) } : {}) };
