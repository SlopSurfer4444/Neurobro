...
async function fixture(t: TestContext, count = 105) {
  ...
  const values = Array.from({ length: count }, (_, i) => new Api.Message({ id: i + 1, date: 100, message: "Synthetic observed row " + (i + 1),
    peerId: new Api.PeerChannel({ channelId: bigInt(123) }), fromId: new Api.PeerUser({ userId: bigInt(456) }) }));
  function source(taskSignal: AbortSignal, onInvoke?: () => Promise<void>) {
    ...
    const reader = createSelfHistoryReader({ binding: { accountId: intent.accountId, peerId: intent.chatId }, signal: taskSignal, references,
      self: new Api.User({ id: bigInt(789), self: true }), peer: new Api.InputPeerChannel({ channelId: bigInt(123), accessHash: bigInt(987) }),
  ...
  function ticket(options: { onInvoke?: () => Promise<void>; onClose?: () => Promise<void> } = {}): StandingIdleHistoryTicket {
    let used = false;
    return Object.freeze({ openTaskReply() { throw Error("unexpected task reply"); }, openHistoryTask(scope) {
